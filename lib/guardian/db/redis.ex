defmodule Guardian.DB.Repo.Redis do
  @moduledoc """
    `Guardian.DB.Repo.Redis` is a repo module that operates Guardian.DB.Token in Redis.
  ...
  def one(query, _opts \\ []) do
    ...
    case MyApp.Redix.command(["GET", key]) do
  ...
  def insert(struct, _opts \\ []) do
    ...
    case MyApp.Redix.command(["SETEX", key, Integer.to_string(expires_in), Jason.encode!(token)]) do
      {:ok, "OK"} ->
        # Adding key to the set `sub` (user_id in JWT) so we can delete all user tokens in one go
        sub = sub_elem(struct)
        MyApp.Redix.command(["SADD", set_name(sub), key])
  ...
  def delete_all(query, _opts \\ []) do
    ...
    {:ok, keys} = MyApp.Redix.command(["SMEMBERS", set_name])
    {:ok, amount_deleted} = MyApp.Redix.command(["DEL", set_name] ++ keys)

    {amount_deleted - 1, :ok} # yeah, vise-versa
  end
  ...
  def delete(model, _opts \\ []) do
    ...
    case MyApp.Redix.command(["DEL", key]) do
  ...
  defp query_param(query, param) do
    (query.wheres |> List.first()).params |> Enum.find(fn i -> i |> elem(1) |> elem(1) == param end) |> elem(0)
  end
end
